# building_manager/building_operations.py
...
class BuildingOperations:
    ...
    def open_building(self, building: Building, action=None):
        map_checked = False
        ...
        while True:
            ...
            if locate(f'pngs/building/{building.name}.png', 0.98):
                print(f"Znaleziono budynek: {building.name}!")
                if action == 1:
    ...
    def check_level(self, building: Building):
        if self.open_building(building, 2):
            time.sleep(1)
